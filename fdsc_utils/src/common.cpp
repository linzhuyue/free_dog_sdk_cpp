...
namespace FDSC{
   std::string lib_version() {
    ...
    float hex_to_float_2B(const std::vector<uint8_t>& byte_arr,int startIndex, int endIndex){
        ...
        uint16_t val = static_cast<uint16_t>((byte_arr[endIndex] << 8) | byte_arr[startIndex]);
        int base = val / 32;
    ...
    void show_in_lowcmd()
    {
        std::cout<<"Before Run in lowlevel mode! You should do the following steps to your dog!!"<<std::endl;
        // std::cout <<"Step1: "<< "L2 + A" << std::endl;
        // std::cout <<"Step2: "<< "L2 + A" << std::endl;
        std::cout <<"Step3: "<< "L2 + B" << std::endl;
        std::cout <<"Step4: "<< "L1 + L2 + START" <<std::endl;
    }
}