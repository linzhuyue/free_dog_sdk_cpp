...
while True:
    ...
    for paket in data:
        ...
        print(f'IMU Quaternion: {hstate.imu.quaternion}%')
        print(f'IMU gyroscope:  {hstate.imu.gyroscope} %')
        print(f'IMU accelerometer:  {hstate.imu.accelerometer} %')
        print(f'IMU rpy:  {hstate.imu.rpy} temperature: {hstate.imu.temperature}%')
        
        print(f'Joint info:\t\t\t--------')
        
        # for i in range(20):
        print(f'Mode: {hstate.motorstate[d["FR_0"]].mode} \t\t q: {hstate.motorstate[d["FR_0"]].q}')
        print(f'q_raw: {hstate.motorstate[d["FR_0"]].q_raw}\t\t dq: {hstate.motorstate[d["FR_0"]].dq}')
        print(f'dq_raw: {hstate.motorstate[d["FR_0"]].dq_raw}\t\t ddq: {hstate.motorstate[d["FR_0"]].ddq}')
        print(f'ddq_raw: {hstate.motorstate[d["FR_0"]].ddq_raw}\t\t tau_est: {hstate.motorstate[d["FR_0"]].tauEst}')
