...
while True:
    ...
    for paket in data:
        ...
        if motiontime % 100 == 0: #Print every 100 cycles
            ...
            print("----FR_2-----")
            print(f'Mode: {lstate.motorState[d["FR_1"]].mode} ')
            print(f'q: {lstate.motorState[d["FR_2"]].q}  \t\t  q_raw: {lstate.motorState[d["FR_2"]].q_raw}')
            print(f'dq: {lstate.motorState[d["FR_2"]].dq} \t\t  dq_raw: {lstate.motorState[d["FR_2"]].dq_raw}')
            print(f'ddq_raw: {lstate.motorState[d["FR_2"]].ddq_raw}\t\t ddq: {lstate.motorState[d["FR_2"]].ddq}')
            print(f'tau_est: {lstate.motorState[d["FR_2"]].tauEst}')
            print("-----FR_1-----")
            print(f'q: {lstate.motorState[d["FR_1"]].q}  \t\t  q_raw: {lstate.motorState[d["FR_1"]].q_raw}')
            print(f'dq: {lstate.motorState[d["FR_1"]].dq} \t\t  dq_raw: {lstate.motorState[d["FR_1"]].dq_raw}')
            print(f'ddq_raw: {lstate.motorState[d["FR_1"]].ddq_raw}\t\t ddq: {lstate.motorState[d["FR_1"]].ddq}')
            print(f'tau_est: {lstate.motorState[d["FR_1"]].tauEst}')
            print('+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=')
